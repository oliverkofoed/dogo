...
func dogoSSH(config *schema.Config, environment *schema.Environment, target string) error {
	...
	fmt.Println(term.BlackBold + runChar(dashes, 30+len(targetResource.Manager.Name)+len(targetResource.Name)) + term.Reset)
	fmt.Println(term.BlackBold + "-----[ connected to " + term.Yellow + environment.Name + "." + targetResource.Name + term.BlackBold + " (" + targetResource.Manager.Name + ") ]-----" + term.Reset)
	fmt.Println(term.BlackBold + runChar(dashes, 30+len(targetResource.Manager.Name)+len(targetResource.Name)) + term.Reset)
	fmt.Println()
